...
const Chats: React.FC<ChatsProps> = ({ currentChat }) => {
  ...
  return (
    <div
      className={`flex flex-col h-full rounded-xl shadow-lg border ${
    ...
    >
      ...
      >
        {onlineUsers.map((user) => (
          <div
            key={user}
            className="flex items-center gap-2 bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full dark:bg-green-700 dark:text-green-300"
          >
        ...
        {loading ? (
          ...
          chats.map((msg, index) => (
            <div
              key={index}
              className={`flex ${
                msg.sender.id === auth.user?.id ? "justify-end" : "justify-start"
              }`}
            >
              <div
